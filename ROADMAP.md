...
- **Council-Timeouts & Performance** — ausgelöst durch einen Codex-Abbruch (`timed out awaiting tools/call after ~300s`) bei einem großen `ask_council`-Lauf. Reihenfolge bewusst „erst messen, dann tunen":
  ...
  - **`tool_timeout_sec` anpassbar machen.** Das Codex-Host-Limit (Abbruchquelle, nicht unser interner Timeout) ist über `tool_timeout_sec` in `~/.codex/config.toml` steuerbar. Installer/Setup-UI sollten es optional mitschreiben/erhöhen können, statt dass man es von Hand ergänzt.
  - **Internen Timeout pro Stage statt pro Call validieren.** Heute sind 120s das Limit *pro Einzelcall*. Sinnvolle Stage-Budgets erst nach der Messung festlegen — Stage 3 gesondert, weil dort die Fallback-Chairmen seriell probiert werden und sich Calls aufsummieren.
  - **Fast Mode / Stage 2 abschaltbar.** Optionaler Schalter (Einstellungen und/oder Tool-Parameter), der das Peer-Ranking überspringt — spart eine ganze Stufe *und* verkürzt den Stage-3-Input.
  - **`max_tokens`-Default überprüfen.** Begrenzt nur die Output-Länge (nicht den `code_context`-Input). Erst anhand der realen Messdaten (Punkt 1) entscheiden, ob 8192 zu hoch ist.
  - **UI-Hinweis bei Timeout-Risiko.** Bei langen Läufen/Timeouts dezent darauf hinweisen, dass man die Modellanzahl (z. B. auf 3) reduzieren kann.

## Product Features After Stabilization
- **Job/Poll-Modus + Teilergebnis-Persistenz für lange `ask_council`-Runs:** `ask_council` bzw. ein neues Start-Tool gibt sofort eine `job_id` zurück; `get_council_result(job_id)` pollt Ergebnis und Status. Das macht Fortschritt host-unabhängig sichtbar und entschärft Tool-Timeouts, sobald reale Läufe >2-3 Minuten problematisch werden. Zugleich der einzige Weg, Teilergebnisse zu **retten**: Bricht der Host-Timeout einen synchronen Call ab, verwirft der Client das Ergebnis (außerhalb unserer Kontrolle) — beim Job-Modus läuft der Council serverseitig weiter und Stage-Ergebnisse werden persistiert, statt verloren zu gehen.
